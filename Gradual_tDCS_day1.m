...
%%baseline 10+10 gradual 40 NF 40 veridical 10
%% initialization
clear;
close all;
clc;
...
base_trials = 10 .* Target_num;     % 80,1-80
...
% 
% base_trials = 1 .* Target_num;     % 80,1-80
% tDCSbase_trials = 1 .* Target_num;     % 80,81-160
% training_trials = 4 .* Target_num;       % 共80*4=320   161-480
% nofeedback_trials=4.* Target_num;       % 共80*4=320   481-800
% veridical_trials =1 .* Target_num;     % 共80*1=80 801-880


% base_trials = 2 .* Target_num;     % 160,1-160
% 
% training_trials = 2 .* Target_num;       % 共80*5=400   161-560
% 
% nofeedback_trials=2.* Target_num;       % 共80*5=400   161-560
% 
% veridical_trials =2 .* Target_num;     % 共80*3=240 561-800
% 
% 
% relearn_trials = 2 .* Target_num;      % 共80*5=400 801-1200
...
text_welcome_words=char('实验第1阶段开始，使绿色光标打中白色目标点','第2阶段，使绿色光标打中白色目标点','第3阶段，使绿色光标打中白色目标点','第4阶段，没有绿点反馈，使手打中白色目标点','最后阶段，使绿色光标打中白色目标点');
...
try
    ...
    for trial_i = 1:trialsNum
        ...
        while(startingFlag == 0)
            ...
            % 实时反馈
            if getReadyFlag == 1
                if  arrivedFlag == 0 && (session_i==1 || session_i==2 || session_i==5) ;                     % veridical
                    rot_angle = 0;
                ...
                elseif session_i==4         %no feedback
                    rot_angle = 0;
                    tempV = current_screen-startingPos_scr;
                    tempL = sqrt(tempV(1)^2+tempV(2)^2);
                    tempA=atan(tempV(2)/tempV(1))*180/pi;    %(-90,90)
...
%                 elseif arrivedFlag == 0 && (session_i==5);             %relearning
%                     rot_angle = 30;
%                     tempV = current_screen-startingPos_scr;
%                     tempL = sqrt(tempV(1)^2+tempV(2)^2);
%                     tempA=atan(tempV(2)/tempV(1))*180/pi;    %(-90,90)
...
%                     set(cursorH,'XData',now_feedback(1),'YData',now_feedback(2),'visible','on');   drawnow;
                ...
                elseif arrivedFlag == 0 && (session_i==3);             %gradual
                   
                    rot_angle = 30/training_trials*(trial_i-tDCSbase_trials-base_trials);
                    ...
                    if  savedData(trial_i,iStep,4) >=T2Sdist;
                      ...
                      if session_i==4
                        ...
                        if Periods(trial_i)>=50 && Periods(trial_i)<=slowlimit
                            wavplay(sound_arrive.*20, 30000,'async');
...
%                     set(SuccessH,'visible','off');drawnow
        ...
        elseif    trial_i==sum(trials_index(1))||trial_i==sum(trials_index(1:3))||trial_i==sum(trials_index(1:4))
...
%     set(instruction_totalBonus,'string',[num2str(totalBonus) ' 分'],'visible','on');
    ...
    save([Subinfo{1} '_day1']);
    
catch
    ShowCursor;
    save([filename '_d1temp']);    % 出错后存储临时文件
