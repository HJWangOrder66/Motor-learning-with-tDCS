...
%%targets_老板修正版_ok_距离起始点一点范围才给反馈
%%NF10 baseline 10 relearning 40 NF 40 veridical 10
%% initialization
clear;
close all;
clc;
cd('C:\Lab\WHJ\gradual+tDCS');
...
WinTabMex(0,w,0);	% Initialize driver, connect it to window ‘wPtr’
pause(2);
...
promptParameters = {'Subject Name', 'Age', 'Gender (F or M?)','Handedness (L or R)','Stimulus'};
defaultParameters = {'Noname', '20','F', 'R','a'};
...
%% Define Plot Items
startSize=round(screen*6);
targetSize=round(screen*4);
successSize=round(screen*4);
cursorSize=round(screen*3);

startPointH = plot(0,0,'y+','MarkerFaceColor','y','MarkerSize',startSize,'visible','off');
TargetPointH = plot(0,0,'wo','MarkerSize',targetSize,'visible','off');
...
prepareTime = 0.5;        % 在起始点附近超过0.8秒后，目标点亮起
Target_num = 8;
...
pre_nofeedback_trials=10.* Target_num;       % 80   1-80
base_trials = 10 .* Target_num;     % 240   81-160
relearn_trials = 40 .* Target_num;      % 共80*4=320 161-480
post_nofeedback_trials=40.* Target_num;       % 共80*4=320   481-800
veridical_trials =10 .* Target_num;     % 共80*1=80 801-880


% pre_nofeedback_trials=1.* Target_num;       % 80   1-80
% base_trials = 1 .* Target_num;     % 240   81-160
% relearn_trials = 4 .* Target_num;      % 共80*4=320 161-480
% post_nofeedback_trials=4.* Target_num;       % 共80*4=320   481-800
% veridical_trials =1 .* Target_num;     % 共80*1=80 801-880



trials_index = [pre_nofeedback_trials base_trials relearn_trials post_nofeedback_trials veridical_trials];
...
block_iPerTrial = [ones(1,sum(trials_index(1))) 2*ones(1,sum(trials_index(2))) 3.*ones(1,sum(trials_index(3))) 4.*ones(1,sum(trials_index(4))) 5.*ones(1,sum(trials_index(5)))];   %to define blocks. Here, same as session
...
text_welcome_words=char('实验第1阶段开始，没有绿点反馈，使手打中白色目标点','第2阶段，使绿色光标打中白色目标点','第3阶段，使绿色光标打中白色目标点','第4阶段，没有绿点反馈，使手打中白色目标点','最后阶段，使绿色光标打中白色目标点');
    ...
    for trial_i = 1:trialsNum
        ...
        while(startingFlag == 0)
            ...
            if (getReadyFlag == 0)
                ...
                if(distanceFromStartPosition<10);
            ...
            % 实时反馈
            if getReadyFlag == 1
                if  arrivedFlag == 0 && (session_i==2||session_i==5) ;                     % veridical
                    rot_angle = 0;
                ...
                elseif session_i==1 || session_i==4           %no feedback
                    rot_angle = 0;
                    tempV = current_screen-startingPos_scr;
                    tempL = sqrt(tempV(1)^2+tempV(2)^2);
                    tempA=atan(tempV(2)/tempV(1))*180/pi;    %(-90,90)
                ...
                elseif arrivedFlag == 0 && (session_i==3);             %relearning
                    rot_angle = 30;
                    tempV = current_screen-startingPos_scr;
                    tempL = sqrt(tempV(1)^2+tempV(2)^2);
                    tempA=atan(tempV(2)/tempV(1))*180/pi;    %(-90,90)
...
%                 elseif arrivedFlag == 0 && (session_i==2);             %
%                    
%                     rot_angle = 30/(40*8)*(trial_i-160);
%                   
%                     tempV = current_screen-startingPos_scr;
%                     tempL = sqrt(tempV(1)^2+tempV(2)^2);
%                     tempA=atan(tempV(2)/tempV(1))*180/pi;    %(-90,90)
...
%                     set(cursorH,'XData',now_feedback(1),'YData',now_feedback(2),'visible','on');   drawnow;
                ...
                if getReadyFlag == 1 && arrivedFlag == 0
                    ...
                    if  savedData(trial_i,iStep,4) >=T2Sdist;
                      ...
                      if session_i==1 || session_i==4
                            set(cursorH,'XData',cursorFinal(1),'YData',cursorFinal(2),'MarkerSize',cursorSize,'visible','off');   drawnow;
                      else
                            set(cursorH,'XData',cursorFinal(1),'YData',cursorFinal(2),'MarkerSize',cursorSize,'visible','on');  hold on; drawnow;
                      end;
                        ...
                        Periods(trial_i) = (savedData(trial_i,trialsEndStep(trial_i),3) - savedData(trial_i,1,3))*1000;   %ms
                        
                        if Periods(trial_i)>=50 && Periods(trial_i)<=slowlimit
                            wavplay(sound_arrive.*10, 30000,'async');
                        elseif   Periods(trial_i)<200  % 过快
                            fast=1;
...
%                     set(SuccessH,'visible','off');drawnow
                    set(cursorH,'visible','off');drawnow
                    set(text_MoveStop,'visible','off');
                    startingFlag = 1;
                    clear tReturn
                end;    % end if(...&&toc...),当返回的时间超过设定时间时，表示整个运动阶段结束，跳出while循环
            end;
        ...
        elseif    trial_i==sum(trials_index(1:2))||trial_i==sum(trials_index(1:3))||trial_i==sum(trials_index(1:4))
