    ...
    for trial_i = 1:trialsNum
        ...
        while(startingFlag == 0)
            ...
            if (getReadyFlag == 0)
                ...
                if(distanceFromStartPosition<5);
                    ...
                    if  toc(tPrepareTic)>prepareTime %-0.5
                        ...
                        if trial_i==160
                            disp('turn on tDCS');%no break in case participants notice anything
                        end;
                        trialStartTime = tic;
            ...
            if getReadyFlag == 1
                ...
                elseif arrivedFlag == 0 && (session_i==3);             %error clamp
                    ...
                    now_feedback = [cos(angle_feedback) sin(angle_feedback)] * tempL + startingPos_scr;
                    now_feedbackEC = [cos(angle_feedbackEC) sin(angle_feedbackEC)] * tempL + startingPos_scr;
                    %                     set(cursorH,'XData',now_feedback(1),'YData',now_feedback(2),'visible','off');   drawnow;
                    if tempL>startSize+targetSize
                        set(doubleH,'XData',now_feedbackEC(1),'YData',now_feedbackEC(2),'visible','on');  drawnow;
                    else
                        set(doubleH,'XData',now_feedbackEC(1),'YData',now_feedbackEC(2),'visible','off');  drawnow;
                    end
                end;
